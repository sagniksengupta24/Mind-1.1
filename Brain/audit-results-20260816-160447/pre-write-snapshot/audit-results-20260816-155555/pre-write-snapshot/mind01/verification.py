    ...
    def _run(self, argv: tuple[str, ...], name: str, blocking: bool) -> VerificationResult:
        ...
        try:
            completed = subprocess.run(
                ...
                shell=False,
                env={"PATH": str(Path(shutil.which(argv[0]) or argv[0]).parent) + ":/usr/bin:/bin", "PYTHONNOUSERSITE": "1"},
            )
            output = (completed.stdout or "")[-self.output_limit :]
            status = "passed" if completed.returncode == 0 else "failed"
            return VerificationResult(name, status, int((time.perf_counter() - start) * 1000), output, "command_failed" if completed.returncode else "", 0.95, blocking and completed.returncode != 0, command=argv)
        except subprocess.TimeoutExpired as exc:
            output = ((exc.stdout or "") if isinstance(exc.stdout, str) else "")[-self.output_limit :]
